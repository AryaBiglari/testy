...
export default function JigSwiper2(props) {
  const {
    trailer,
    jig,
    eng,
  ...
  const handleUpdateCurrentStageStatuses = async (stageIndex, newStatus) => {
    ...
    if (newStatus === "completed and confirmed") {
      const isFinalStage = stageIndex === trailer?.boxData?.stages?.length - 1;
  ...
  React.useEffect(() => {
    const initializeState = () => {
      let stages = props?.trailer?.boxData?.stages;
      let firstIncompleteStageIndex = stages.findIndex(
        (stage) => stage.status !== "completed and confirmed"
      );

      if (firstIncompleteStageIndex === -1) {
        
        firstIncompleteStageIndex = stages.length - 1;
      }

      setStage(firstIncompleteStageIndex + 1);

      let activeStageIndex = firstIncompleteStageIndex;
      console.log("asi", activeStageIndex);
  ...
  const ActiveEmployeesBox = () => (
    ...
    >
      ...
      {activeEmployees?.length > 0 ? (
        ...
        <Typography>No active employees.</Typography>
      )}
    </Box>
  );
    ...
    >
      ...
      >
        ...
        <Button
          ...
          onClick={() => {
            props.handleJigRemoval(props.trailer._id);
            setIsConfirmingRemoval(false); // Optional: Reset the confirmation state
          }}
        >
          Yes
        </Button>
      ...
      {isActive && !isCompleted && (
        ...
        >
          {/* Active Employees Section - Integrated */}
          {isActive && !isCompleted && <ActiveEmployeesBox />}
        </Box>
      )}
